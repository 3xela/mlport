...
static __global__ void reduction_kernel_v0(const float* x, float* block_sums , int n) {
...
static __global__ void reduction_kernel_v1(const float* x, float* block_sums , int n) {
    __shared__ float smem[256];
    const int global_tid = blockDim.x * blockIdx.x + threadIdx.x;
    const int tid = threadIdx.x;
    const int grid_stride = gridDim.x * blockDim.x;
...
static __global__ void reduction_kernel_v2(const float* x, float* block_sums , int n) {
...
static __global__ void reduction_kernel_v3(const float* x, float* block_sums , int n) {
    // total # of warps: #blocks * #threads/block / 32 = 152 * 256 / 32 = 1216
    __shared__ float smem[8];
    const int global_tid = blockDim.x * blockIdx.x + threadIdx.x;
    const int tid = threadIdx.x;
...
void reduction_launch_v3(void* p) {
    auto* c = (ReductionCtx*)p;
    int grid = 152;
    reduction_kernel_v3<<<grid, 256>>>(c->x, c->block_sums ,c->n);
}
